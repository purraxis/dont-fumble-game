...
export async function generateFeedbackForAnswer(data: FeedbackInput): Promise<Feedback> {
  const input = InputSchema.parse(data);

  if (process.env.E2E_FAKE_AI === "true") {
    return {
      score: 84,
      verdict: "The Terrarium Upgrade Plan",
      what_worked: "Specific, reassuring, and just unhinged enough to survive the group chat.",
      what_fumbled: "Could use one extra ounce of romance, but nobody is calling HR.",
      better_answer: `I would choose you in every form, including this ${input.packName} disaster.`,
    };
  }
...
export const generateFeedback = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<Feedback> => generateFeedbackForAnswer(data));
